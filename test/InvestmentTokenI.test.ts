...
describe("InvestmentTokenI Contract", async () => {
  let token: any;
  let registry: any;
  let OWNER: any;
  let ADDR1: any;
  ...
  beforeEach(async () => {
    ...
    const Token = await ethers.getContractFactory("InvestmentTokenI");
    [OWNER, ADDR1, ADDR2] = await ethers.getSigners();

    registry = await Registry.deploy();
    token = await Token.deploy("Investment Token", "ITK", registry.address);

    await token.initializeSupply(ONE_MILLION);
  });
  ...
  describe("InitializeSupply", () => {
    it("should initializeSupply failed when supply has initialized", async () => {
      await expect(token.initializeSupply(ONE_MILLION)).to.be.revertedWith(
        "InvestmentTokenI: supply has initialized"
      );
    });

    it("should initializeSupply failed when caller is not the owner", async () => {
      await expect(
        token.connect(ADDR1).initializeSupply(ONE_MILLION)
  ...
  describe("Transfer", () => {
    ...
    it("Should transfer failed when sender are not allowlisted account", async () => {
      ...
      ).to.be.revertedWith("InvestmentTokenI: account are not allowlisted");
    });
  ...
  describe("Burnable", () => {
    ...
    it("Should burn tokens failed when account are not allowlisted", async () => {
      ...
      await expect(token.connect(ADDR1).burn(ONE_THOUSAND)).to.be.revertedWith(
        "InvestmentTokenI: account are not allowlisted"
      );
    });
